...
const cardStyles = `
    ...
    .card-footer { position: absolute; bottom: 0; width: 100%; height: 32px; background: #e8f5e9; border-top: 1.5px solid #4caf50; display: flex; align-items: center; justify-content: center; padding: 0 10px; box-sizing: border-box; }
    .footer-fsc { font-weight: 900; font-size: 1.2rem; color: #0000ff; letter-spacing: 0.5px; }
...
function renderCardsOnPage(data) {
    ...
    const generateHtml = (id, title, list) => {
        const rows = list.map(m => `<tr><td style="width:15px; color:#aaa;">${m.sno}</td><td style="font-weight:700;">${m.name}</td></tr>`).join('');
        return `
                ...
                <div class="card-content-split">
                    <div class="info-side">
                        ...
                        <div style="margin-top: 8px; text-align: center;">
                            <img src="${qrImgSrc}" style="height: 65px; width: 65px; border: 1px solid #ddd;" alt="QR Code">
                        </div>
                    </div>
                    <div class="list-side">
